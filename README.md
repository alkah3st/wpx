...
For example:
	...
	<section class="recent-articles"> <--! has semantic meaning, is a "section" of the document for Recent Articles -->
...
**Mobile.** You will notice that the ```/mobile/``` folder mirrors the contents of the ```/globals/``` folder. Under the ```/mobile/``` directory, there is another base.scss and another ```/modules/``` folder, etc. If there is a need to define mobile breakpoints for any selectors in any SASS file, the breakpoints and their selectors should be defined in the corresponding mobile SASS partial. For example, if you created a ```primary-nav.scss``` in ```/modules/``` and need to define mobile breakpoints, you should create an additional primary-nav.scss in ```/mobile/modules/primary-nav.scss```. In mobile SASS partials, breakpoints should be ordered from highest values to smallest value (down to 320 pixels). 
...
The module object's naming pattern is best expressed as: 
	...
	module prefix + name (.module-related) 
		> name + context (.related-inner)
			> name + context (.related-description)

The topmost selector's class name should always be prefixed with the word "module", and nested selectors should be prefixed with the module's unique name.
...
Use the @font-size mixin to render fonts in both rems and pixels.

		h1 {
			@include font-size("16px");
		}

This will render pixels for browsers that don't support rem:

...
* ```templates```, for WordPress page templates, block templates, and api templates
...
- **schema.php** is for schema markup
...
All the SASS files for the project are in ```/styles/sass/``` folder, organized like so:
	...
	globals
		...
		- blocks.scss // all styles to override core Gutenberg blocks
		- buttons.scss // all styles to override core Gutenberg buttons (and for custom buttons) 
	...
	templates // for styles concentrated to static templates
...
**Vendor**. The ```/vendor/``` folder is meant for including any SASS partials or CSS provided by third party plugins, and will get compiled with everything else. The styles in ```/vendor/icons/``` as well as the ```sprites.scss``` sheet are generated by the built script. Just rename any vendor CSS to SASS and drop in this folder. 
...
To declare a color in an element, use the WP variables established in theme.json. For example, if you set: 
...
Then you can reference that color as `var(--wp--preset--color--blue)`.

### Viewport Font Sizes

Use the @font-size mixin to render fonts responsively. Always specify your sizes in pixels, and they will get converted to rem as they are interpolated from a min to max size (using clamp()).

		h1 {
			@include font-size($size: 16px, $min: 14px, $max: 22px, $context: 1000px);
		}

At a minimum, you can just specify the starting size and the minimum size.

		h1 {
			@include font-size(16px, 14px);
		}

This will render a default size, a minimum size, a maximum size:

		font-size: clamp(14px, 1.6vw, 16px);

### Context Mixin & Responsive Layouts

I tend to standardize my "context" in a responsive design. Context in this sense means the maximum width of the content area that the designer intended. Usually this is something near 1200 pixels. The context widths are set in theme.json as:
...
Or you can call `context($max-width: var(--wp--style--global--content-size), $width: 90%);` and it will output:
...
You can then size elements based on a percent of that context responsively.
...
While ```@include context()``` defines the standard page width for the flow of content, ```@include width-wide()``` optionally defines a size when a block is set to "wide" width, meaning it breaks out of the flow about 120%. This is 1600px (matching the value of ```--wp--style--global--wide-size``` vs. the standard 1200px (the value of ```--wp--style--global--content-size```).

You can force a block or component to break out of the page context by using the ```width-wide()``` or ```explode-width()``` mixins:

    .my-block {
    	@include width-wide($width: 120%, $margin: -10%, $breakeven: 1000px);
    }
    
The parameters are optional, and the ```$breakeven``` is the viewport where we should set the container back to 100%. Results in:
...
You may want to redefine the default for ```$breakeven``` at which the block goes back to 100% width to suit the needs of the design.
